...
def get_agent_description() -> Dict[str, Dict[str, Any]]:
    """
    Return structured agent descriptions with capabilities and guidelines. Edit this function to change how the planner/executor reason about agents.
    """
    return {
        "web_researcher": {
            "name": "Web Researcher",
            ...
            "limitations": "Cannot access private/interal comopany data",
            "output_format": "Raw research data and finding from public sources"
        },
        "cortex_researcher": {
            "name": "Cortext Researcher",
            ...
            "output_format": "For structured requests, return the exact fields and include SQL when applicable; For unstructured requests, return concise relevant exerpts with citations.",
        },
        "chart_generator": {
            ...
            "limitations": "Requires structured input from previous steps",
            "output_format": "Visual charts and graphs",
            "position_requirement": "Must be used as final step after data gathering is completed",
        },
        "chart_summarizer": {
...
def agent_list_for_planning(state: State | None = None) -> str:
    """
    ...
    for agent_key, details in descriptions.items():
        if agent_key not in enabled_list:
            continue
        agents.append(f" • `{agent_key}`- {details['capability']}")
    return "\n".join(agents)
...
def format_agent_guidelines_for_planning(state: State | None = None) -> str:
    ...
    if "web_researcher" in enabled:
        guidelines.append(f"- Use `web_researcher` when {descriptions['web_researcher']['use_when'].lower()}.")
    ...
    if "chart_generator" in enabled:
        ...
        guidelines.append(f"- **Include  `chart_generator` _only_if {chart_desc['use_when'].lower()}**. If included, `chart_generator` must be : {chart_desc['position_requirement'].lower()}. Visualizations should include all of the data from the previous steps that is reasonable for the chart type.{cs_hint}")
...
def format_agent_guidelines_for_executor(state: State | None = None) -> str:
    ...
    if "cortex_researcher" in enabled:
        ...
        guidelines.append(f"- Use `cortex_researcher` when {cortex_desc['use_when'].lower()}.")
...
def plan_prompt(state: State) -> HumanMessage:
    ...
    agent_list = agent_list_for_planning(state)
    ...
    enabled_for_planner = [
        a for a in enabled_list if a in ("web_researcher", "cortex_researcher", "chart_generator", "synthesizer")
    ]
    ...
    if replan_flag:
        prompt += f"""
        The current plan needs revision becuase: {replan_reason}
...
def executor_prompt(state: State) -> HumanMessage:
    ...
    latest_plan: Dict[str, Any] = state.get("plan", {})
